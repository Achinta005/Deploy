...
//Only Change Here For New Model Addition
...
export default function MLModelsHomepage() {
    ...
    const fetchAndMergeModels = async () => {
        setIsLoading(true);
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL_SERVER}/projects`);
            ...
            <div className="relative z-10">
                ...
                <div className="max-w-7xl mx-auto px-4 pb-24">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        {isLoading ? (
                            // FIX: Changed Array(4) to Array(4).fill(0) to correctly create an array 
                            // that can be mapped over in JSX.
                            Array(4).fill(0).map((_, index) => (
                                <div key={index} className="h-64 bg-white/5 backdrop-blur-md border border-white/20 rounded-2xl p-6 animate-pulse">
                            ...
                            models.map((model) => {
                                ...
                                return (
                                    <a href={model.path} key={model.id} target="_blank" rel="noopener noreferrer">
                                        <div
                        ...
                        {!isLoading && emptySlots.map((_, index) => (
